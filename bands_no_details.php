...
<body>
  ...
  <div class="standard colored">
    <?php echo "Entries: " . count($bandFiles) . ". Sorting is chronological."; ?>
  </div>
  ...
  function generateBandLink($file_path)
  {
    ...
    foreach ($bands_array as $band_name) {
      ...
      $band_url = 'band.php?a=' . urlencode(strtolower(str_replace('__', '_', str_replace(' ', '_', str_replace('&', 'and', $band_name_clean)))));

      // Output the band name as a link
      echo '<a href="' . $band_url . '">' . $band_name . '</a><br>';
    }
  }
  ?>
  ...
  <table class="standard">
    <tr>
      <?php
      // Associative array with artist names and file paths
      $artists = array(
