...
$release = isset($_GET['a']) ? $_GET['a'] : '';
$filePath = 'releases/band/' . $release . '.txt';

if (!is_file($filePath)) {
  http_response_code(404);
  die('<h2>Release file not found: ' . htmlspecialchars($release, ENT_QUOTES, 'UTF-8') . '</h2>');
}
...
$pagetitle = $albumTitle !== '' ? $albumTitle : $release;

function find_release_cover($slug)
{
  $extensions = ['png', 'webp', 'jpg', 'jpeg'];

  foreach ($extensions as $ext) {
    $large = 'photos_releases_large/band/' . $slug . '.' . $ext;
  ...
  foreach ($extensions as $ext) {
    $small = 'photos_releases/band/' . $slug . '.' . $ext;
...
$imgThumb = find_release_cover($release);
$thumb = $imgThumb;
...
$nextRelease = basename($releases[$nextIndex], '.txt');
$prevRelease = basename($releases[$prevIndex], '.txt');

$releaseSlugs = array_map(function ($item) {
...
<body>
  ...
  <div class="skip-buttons colored">
    <span id="skip-left-btn" class="skip-button">
      <a href="release_band.php?a=<?= htmlspecialchars($prevRelease, ENT_QUOTES, 'UTF-8') ?>" title="Previous">
        <svg class="icons">
    ...
    <span id="skip-right-btn" class="skip-button">
      <a href="release_band.php?a=<?= htmlspecialchars($nextRelease, ENT_QUOTES, 'UTF-8') ?>" title="Next">
        <svg class="icons">
  ...
  <script>
    let isPlaying = localStorage.getItem('isPlayingReleaseBand') === 'true';
    let playInterval;
    let shouldResumeOnNextPage = false;
    const playButton = document.getElementById('playPauseBtn');

    const releases = <?= json_encode($releaseSlugs) ?>;
    const currentIndex = releases.indexOf(<?= json_encode($release) ?>);

    function navigateToNextRelease() {
      const nextRelease = releases[(currentIndex + 1) % releases.length];
      shouldResumeOnNextPage = true;
      localStorage.setItem('currentReleaseBand', nextRelease);
      window.location.href = 'release_band.php?a=' + encodeURIComponent(nextRelease);
    }
    ...
    function togglePlay() {
      if (isPlaying) {
        ...
        localStorage.setItem('isPlayingReleaseBand', 'false');
      } else {
        playInterval = setInterval(navigateToNextRelease, 5000);
        setPlayIcon('pause');
        isPlaying = true;
        shouldResumeOnNextPage = false;
        localStorage.setItem('isPlayingReleaseBand', 'true');
      }
    }
  ...
  <div class="top-line">
    ...
    <svg class="icons show-pointer no-margin"
      onclick="toggleDiv('releaseBandCoverContainer')"
  ...
  <div class="image-container" id="releaseBandCoverContainer">
  ...
  <div class="top-line">
    ...
    <svg class="icons show-pointer no-margin"
      onclick="toggleDiv('release-band-tracklist-container')"
  ...
  <div class="standard" id="release-band-tracklist-container">
    <?php
    $tracklistFile = 'releases_tracklists/band/' . $release . '.txt';
  ...
  <div class="top-line">
    ...
    <svg class="icons show-pointer no-margin"
      onclick="toggleDiv('release-band-comments-container')"
  ...
  <div class="standard justify center-on-medium-screen" id="release-band-comments-container">
    <?php
    $commentsFile = 'releases_comments/band/' . $release . '.txt';
